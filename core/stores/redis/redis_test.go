...
func TestRedis_GetSet(t *testing.T) {
	runOnRedis(t, func(client *Redis) {
		...
		_, err = client.GetSetCtxWithNilErr(context.Background(), "hello", "world")
...
func TestRedis_SetGetDel(t *testing.T) {
	runOnRedis(t, func(client *Redis) {
		...
		assert.Nil(t, err)
		assert.Equal(t, 1, ret)
		_, err = client.GetCtxWithNilErr(context.Background(), "hello")
		assert.Equal(t, err, Nil)
	})
}
...
func TestRedis_WithDB(t *testing.T) {
	runOnRedis(t, func(client *Redis) {
		err := New(client.Addr, WithDB(1)).Ping()
		assert.NotNil(t, err)
	})
}

func TestRedis_WithPoolSize(t *testing.T) {
	runOnRedis(t, func(client *Redis) {
		err := New(client.Addr, WithPoolSize(1)).Ping()
		assert.NotNil(t, err)
	})
}

func TestRedis_WithMinIdleConns(t *testing.T) {
	runOnRedis(t, func(client *Redis) {
		err := New(client.Addr, WithMinIdleConns(1)).Ping()
		assert.NotNil(t, err)
	})
}

func TestRedis_WithMaxIdleConns(t *testing.T) {
	runOnRedis(t, func(client *Redis) {
		err := New(client.Addr, WithMaxIdleConns(1)).Ping()
		assert.NotNil(t, err)
	})
}

func TestRedis_WithMaxActiveConns(t *testing.T) {
	runOnRedis(t, func(client *Redis) {
		err := New(client.Addr, WithMaxActiveConns(1)).Ping()
		assert.NotNil(t, err)
	})
}
...
func TestRedis_RawExec(t *testing.T) {
	t.Run("raw ping", func(t *testing.T) {
		getfn := func(ctx context.Context, rds RedisNode) error {
		...
		runOnRedis(t, func(client *Redis) {
			err := client.RawExec(context.Background(), getfn)
			assert.NoError(t, err)
		})
	})

	t.Run("raw ping error", func(t *testing.T) {
		getfn := func(ctx context.Context, rds RedisNode) error {
			_, err := rds.Ping(ctx).Result()
			if err != nil {
				return err
