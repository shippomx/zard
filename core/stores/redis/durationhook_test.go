...
import (
	...
	"github.com/shippomx/zard/core/logx/logtest"
	"github.com/shippomx/zard/core/trace/tracetest"
...
func TestHookProcessCase1(t *testing.T) {
	...
	err := defaultDurationHook.ProcessHook(func(ctx context.Context, _ red.Cmder) error {
...
func TestHookProcessPipelineCase1(t *testing.T) {
	...
	err := defaultDurationHook.ProcessPipelineHook(func(_ context.Context, _ []red.Cmder) error {
		return nil
	})(context.Background(), nil)
	assert.NoError(t, err)

	err = defaultDurationHook.ProcessPipelineHook(func(ctx context.Context, _ []red.Cmder) error {
		assert.Equal(t, "redis", tracesdk.SpanFromContext(ctx).(interface{ Name() string }).Name())
		return nil
