...
import (
	...
	"github.com/shippomx/zard/core/errorx"
...
const (
	...
	defaultMaxConnAge    = time.Second * 200
)
...
type (
	...
	RedisNode interface {
		red.Cmdable
		Do(ctx context.Context, args ...interface{}) *red.Cmd
		red.BitMapCmdable
	}
...
func NewRedis(conf RedisConf, opts ...Option) (*Redis, error) {
	...
	if conf.Type == ClusterType {
		...
		if conf.RouteByLatency {
			opts = append([]Option{WithRouteByLatency()}, opts...)
		}
		if conf.RouteRandomly {
			opts = append([]Option{WithRouteRandomly()}, opts...)
		}
		if conf.SingleReplicaSet {
			opts = append([]Option{WithSingleReplicaSet()}, opts...)
		}
	}
	if len(conf.Pass) > 0 {
	...
	if conf.EnableBreaker {
		opts = append([]Option{WithEnableBrk()}, opts...)
	}
	if conf.DB != 0 {
	...
	if conf.MaxActiveConns != 0 {
		opts = append([]Option{WithMaxActiveConns(conf.MaxActiveConns)}, opts...)
	}
	if conf.MaxIdleConns != 0 {
		opts = append([]Option{WithMaxIdleConns(conf.MaxIdleConns)}, opts...)
	}
...
func (s *Redis) GetWithNilErr(key string) (string, error) {
	return s.GetCtxWithNilErr(context.Background(), key)
}
...
func (s *Redis) GetCtxWithNilErr(ctx context.Context, key string) (string, error) {
	conn, err := getRedis(s)
	if err != nil {
		return "", err
	}
...
func (s *Redis) GetSetWithNilErr(key, value string) (string, error) {
	return s.GetSetCtxWithNilErr(context.Background(), key, value)
}
...
// GetSetCtx is the implementation of redis getset command.
func (s *Redis) GetSetCtxWithNilErr(ctx context.Context, key, value string) (string, error) {
	conn, err := getRedis(s)
	if err != nil {
		return "", err
	}
...
// Close is the implementation of redis close command.
// This function should not be called in production code,
// because the underlying redis client is managed by the framework.
// Calling this function will lead to unexpected behavior.
func (s *Redis) Close() {
	conn, err := getRedis(s)
	if err != nil {
...
//
//nolint:revive // can't rename method
...
type rawExec func(ctx context.Context, rds RedisNode) error

// RawExec executes a raw Redis command on  Redis nodes.
//
// The function first checks if the circuit breaker is open, if it is, the
// error returned will be the last error from the circuit breaker. If the
// circuit breaker is closed, it will call the getRedis function to get a
// Redis node, and then call the exec function with the context, Redis node,
// and any other arguments.
//
// The last returned error is the error from executing the exec function.
// If the exec function returns an error, the circuit breaker will be
// notified with the error so it can open the circuit breaker.
//
// The function returns an error if the circuit breaker is open, or if the
// exec function returns an error.
func (s *Redis) RawExec(ctx context.Context, exec rawExec) error {
	// Check if the circuit breaker is open. If it is, return the last error
	// from the breaker.
	err := s.brk.DoWithAcceptable(func() error {
		...
		// Execute the exec function with the context, Redis node, and any
		// other arguments. If an error occurs, return the error.
		err = exec(ctx, rds)
		if err != nil {
			return err
		}
...
// Set route ploicy to  bylatency
func RouteByLatency() Option {
	return func(r *Redis) {
		r.RouteByLatency = true
	}
}

// Set route ploicy to randomly
func RouteRandomly() Option {
	return func(r *Redis) {
		r.RouteRandomly = true
	}
}

// Set single node
func SingleReplicaSet() Option {
	return func(r *Redis) {
		r.SingleReplicaSet = true
	}
}
...
func WithEnableBrk() Option {
	return func(r *Redis) {
		r.EnableBrk = true
	}
}

func WithDB(db uint64) Option {
	return func(r *Redis) {
		r.DB = db
	}
}
...
func WithSingleReplicaSet() Option {
	return func(r *Redis) {
		r.SingleReplicaSet = true
	}
}
...
func WithMaxActiveConns(size int) Option {
	return func(r *Redis) {
		r.maxActiveConns = size
	}
}
