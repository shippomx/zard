...
import (
	...
	"github.com/shippomx/zard/core/sysx"
)
...
func Warnf(format string, v ...any) {
	if shallLog(WarnLevel) {
		writeWarn(fmt.Sprintf(format, v...))
	}
}

// writeWarn writes v into access log with json content.
func Warnv(v any) {
	if shallLog(WarnLevel) {
		writeWarn(v)
	}
...
// writeWarn writes v into warn log.
// Not checking shallLog here is for performance consideration.
// If we check shallLog here, the fmt.Sprint might be called even if the log level is not enabled.
// The caller should check shallLog before calling this function.
func writeWarn(val any, fields ...LogField) {
	getWriter().Warn(val, addCaller(fields...)...)
}
