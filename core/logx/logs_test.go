...
func (mw *mockWriter) Warn(v any, fields ...LogField) {
	mw.lock.Lock()
	defer mw.lock.Unlock()
...
func TestStructedLogWarn(t *testing.T) {
	w := new(mockWriter)
	old := writer.Swap(w)
	defer writer.Store(old)

	doTestStructedLog(t, levelWarn, w, func(v ...any) {
		Warn(v...)
	})
}

func TestStructedLogWarnf(t *testing.T) {
	w := new(mockWriter)
	old := writer.Swap(w)
	defer writer.Store(old)
...
func TestStructedLogWarnv(t *testing.T) {
	w := new(mockWriter)
	old := writer.Swap(w)
	defer writer.Store(old)
...
func TestStructedLogWarnw(t *testing.T) {
	w := new(mockWriter)
	old := writer.Swap(w)
	defer writer.Store(old)

	doTestStructedLog(t, levelWarn, w, func(v ...any) {
		Warnw(fmt.Sprint(v...), Field("foo", "bar"))
	})
}
...
func TestLogLevelSetup(t *testing.T) {
	...
	for level := range data {
		for levelFunc := range data {
			tt = append(tt, args{
				name:     fmt.Sprintf("level: %s, levelFunc: %s", level, levelFunc),
				level:    level,
				fn:       data[levelFunc].fn,
				isLogged: data[levelFunc].level >= data[level].level,
			})
		}
	}
	count := 0
	for _, tc := range tt {
		...
		defer writer.Store(old)
		tc.fn("hello")
		assert.Equal(t, tc.isLogged, w.builder.Len() > 0, tc.name)
	}
	assert.Equal(t, len(data)*len(data), count)
}
