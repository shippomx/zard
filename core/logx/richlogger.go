...
import (
	...
	"github.com/shippomx/zard/internal/trace"
)
...
func (l *richLogger) Warnv(v any) {
	if shallLog(WarnLevel) {
		l.warn(v)
	}
}

func (l *richLogger) Warnw(msg string, fields ...LogField) {
	if shallLog(WarnLevel) {
		l.warn(msg, fields...)
	}
}
...
func (l *richLogger) buildFields(fields ...LogField) []LogField {
	...
	userID, ok := l.ctx.Value(UserIDContextKey).(string)
	if ok {
		fields = append(fields, Field(userIDKey, userID))
	}
...
func (l *richLogger) warn(v any, fields ...LogField) {
	if shallLog(WarnLevel) {
		getWriter().Warn(v, l.buildFields(fields...)...)
	}
}
