...
func newFileWriter(c LogConf) (Writer, error) {
	...
	// Note(Roby): warning is not an error, so save it into accessFile like
	// level `info`.
	if warnLog, err = createOutput(accessFile); err != nil {
		return nil, err
	}
...
func (w *concreteWriter) Warn(v any, fields ...LogField) {
...
func (n nopWriter) Warn(_ any, _ ...LogField) {
}
...
func wrapLevelWithColor(level string) string {
	...
	case levelDebug:
		// Note(Roby): Yellow catches people's attention the most, and debug logs
		// often occupy the vast majority of the journal. So I changed the
		// color of levelDebug to white.
		colour = color.FgWhite
	case levelStat:
