...
func TestWarnf(t *testing.T) {
	buf := logtest.NewCollector(t)
	file, line := getFileLine()
	Warnf(context.Background(), "foo %s", "bar")
	assert.True(t, strings.Contains(buf.String(), fmt.Sprintf("%s:%d", file, line+1)))
}

func TestWarnv(t *testing.T) {
	buf := logtest.NewCollector(t)
	file, line := getFileLine()
	Warnv(context.Background(), "foo")
	assert.True(t, strings.Contains(buf.String(), fmt.Sprintf("%s:%d", file, line+1)))
}
...
func TestWarnw(t *testing.T) {
	buf := logtest.NewCollector(t)
	file, line := getFileLine()
	Warnw(context.Background(), "foo", Field("a", "b"))
	assert.True(t, strings.Contains(buf.String(), fmt.Sprintf("%s:%d", file, line+1)))
}
