...
import (
	...
	"github.com/shippomx/zard/core/fs"
	"github.com/shippomx/zard/core/hash"
...
func TestInitDefaultInK8s(t *testing.T) {
	t.Run("NACOS_CONFIG_FILE environment variable is set", func(t *testing.T) {
		// Set the NACOS_CONFIG_FILE environment variable to a non-existent file
		os.Setenv("NACOS_CONFIG_FILE", "/path/to/nonexistent.yaml")

		// Call the InitDefault function
		err := InitDefaultInK8s(nil)

		// Assert that the function returns an error
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "nacos config no exist: etc/nacos.yaml, set nacosconfig via env `NACOS_CONFIG_FILE`")

		// Reset the NACOS_CONFIG_FILE environment variable
		os.Unsetenv("NACOS_CONFIG_FILE")
	})

	t.Run("NACOS_CONFIG_FILE environment variable is not set, etcFiles is not empty", func(t *testing.T) {
		// Create a temporary directory for etcFiles
		tmpDir := createTempDir(t)
		defer os.RemoveAll(tmpDir)

		// Create a temporary file in etcFiles
		tmpFile, err := os.Create(filepath.Join(tmpDir, "test.yaml"))
		assert.NoError(t, err)
		// write nacos config to tmpfile
...
`)
		...
		// Call the InitDefault function
		err = InitDefaultInK8s(&rest.RestConf{})

		// Assert that the function returns no error
		assert.ErrorContains(t, err, "get config from nacos client error")
	})

	t.Run("NACOS_CONFIG_FILE environment variable is not set, etcFiles is empty", func(t *testing.T) {
		// Create a temporary directory for etcFiles
		tmpDir := createTempDir(t)
		defer os.RemoveAll(tmpDir)
		...
		// Assert that the function returns an error
		assert.Error(t, err)
		assert.EqualError(t, err, "nacos config no exist: etc/nacos.yaml, set nacosconfig via env `NACOS_CONFIG_FILE`")
	})

	t.Run("GetNacosConfig function returns nil", func(t *testing.T) {
		// Call the InitDefault function
		err := InitDefaultInK8s(nil)
