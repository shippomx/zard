...
import (
	...
	"github.com/shippomx/zard/zrpc/internal/balancer/p2c"
	"github.com/shippomx/zard/zrpc/internal/clientinterceptors"
	"github.com/shippomx/zard/zrpc/resolver"
	"google.golang.org/grpc"
	"google.golang.org/grpc/credentials"
	"google.golang.org/grpc/credentials/insecure"
	_ "google.golang.org/grpc/xds" // required for side-effects: registers the xds driver
)
...
func (c *client) buildDialOptions(opts ...ClientOption) []grpc.DialOption {
	...
	// xds must be used in secure mode
	if !cliOpts.Secure || os.Getenv("GRPC_XDS_BOOTSTRAP") != "" {
...
func (c *client) buildUnaryInterceptors(timeout time.Duration) []grpc.UnaryClientInterceptor {
	...
	if c.middlewares.Validate {
		interceptors = append(interceptors, clientinterceptors.ValidateInterceptor)
	}
	if c.middlewares.UserID {
		interceptors = append(interceptors, clientinterceptors.UserIDInterceptor)
	}
	return interceptors
}
