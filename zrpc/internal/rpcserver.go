...
import (
	...
	"github.com/shippomx/zard/zrpc/internal/serverinterceptors"
...
func (s *rpcServer) buildUnaryInterceptors() []grpc.UnaryServerInterceptor {
	...
	if s.middlewares.Validate {
		interceptors = append(interceptors, serverinterceptors.ValidateInterceptor)
	}
	if s.middlewares.UserID {
		interceptors = append(interceptors, serverinterceptors.UserIDInterceptor)
	}
