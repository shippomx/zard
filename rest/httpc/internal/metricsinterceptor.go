...
var (
	MetricClientReqDur = metric.NewHistogramVec(&metric.HistogramVecOpts{
		...
		Labels:    []string{"gz_version", "name", "method", "url"},
	...
	MetricClientReqCodeTotal = metric.NewCounterVec(&metric.CounterVecOpts{
		...
		Labels:    []string{"gz_version", "name", "method", "url", "code"},
	})
)
...
func MetricsInterceptor(name string, pr MetricsURLRewriter) Interceptor {
	return func(r *http.Request, _ bool) (*http.Request, ResponseHandler) {
		startTime := timex.Now()
		return r, func(resp *http.Response, err error) {
			...
			MetricClientReqDur.ObserveFloat(float64(timex.Since(startTime))/float64(time.Millisecond), utils.BuildVersion, name, method, path)
			MetricClientReqCodeTotal.Inc(utils.BuildVersion, name, method, path, strconv.Itoa(code))
		}
	}
}
