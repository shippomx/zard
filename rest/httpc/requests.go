...
import (
	...
	"github.com/shippomx/zard/rest/httpc/internal"
...
func DoWithoutLogging(ctx context.Context, method, url string, data any) (*http.Response, error) {
	req, err := buildRequest(ctx, method, url, data)
	if err != nil {
		return nil, err
	}

	return DoRequestWithoutLogging(req)
}
...
func DoRequestWithoutLogging(r *http.Request) (*http.Response, error) {
	return request(r, defaultClient{
		enableLogging: false,
	})
}
...
func (c defaultClient) enableLogger() bool {
...
func request(r *http.Request, cli client) (*http.Response, error) {
	...
	for i, interceptor := range interceptors {
		...
		r, h = interceptor(r, cli.enableLogger())
		respHandlers[i] = h
	}
