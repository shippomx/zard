...
import (
	...
	"github.com/shippomx/zard/tools/goctl/api/parser/g4/ast"
	"github.com/shippomx/zard/tools/goctl/api/parser/g4/gen/api"
	"github.com/shippomx/zard/tools/goctl/api/spec"
	"github.com/shippomx/zard/tools/goctl/pkg/env"
	apiParser "github.com/shippomx/zard/tools/goctl/pkg/parser/api/parser"
)
...
func (p parser) fillTypes() error {
	for _, item := range p.ast.Type {
		...
		case *ast.TypeAlias:
			...
			p.spec.Types = append(p.spec.Types, spec.DefineStruct{
				RawName:   v.Name.Text(),
				AliasType: v.DataType.Expr().Text(),
				Docs:      p.stringExprs(v.Doc()),
			})
		default:
			return fmt.Errorf("fill types init unknown type %+v", v)
		}
	}
	...
	for _, item := range p.spec.Types {
		...
		case spec.DefineStruct:
			if v.AliasType != "" {
				t, valid := ValidateAliasType(v.AliasType)
				if !valid {
					for _, tt := range t {
						_, err := p.findDefinedType(tt)
						if err != nil {
							return fmt.Errorf("type %v not defined", tt)
						}
					}
				}
			}
		...
		default:
			return fmt.Errorf("fill types append unknown type %+v", v)
		}
	}
	p.spec.Types = types
...
// ValidateAliasType validates whether the alias type is supported.
func ValidateAliasType(name string) ([]string, bool) {
	...
	// any and interface{} are supported
	if name == "any" || name == "interface{}" {
		return []string{}, true
	}

	// slice types are supported
	if strings.HasPrefix(name, "[]") {
		return ValidateAliasType(name[2:])
	}

	// pointer types are supported
	if strings.HasPrefix(name, "*") {
		return ValidateAliasType(name[1:])
	}
	...
	if strings.HasPrefix(name, "map[") {
		if strings.Contains(name, "]") {
			...
			key, keyFlag := ValidateAliasType(name[4:strings.Index(name, "]")])
			// validate the value type
			value, valueFlag := ValidateAliasType(name[strings.Index(name, "]")+1:])
			// combine the results
			return append(key, value...), keyFlag && valueFlag
		}
	}
