...
func dockerCommand(_ *cobra.Command, _ []string) (err error) {
	...
	base := varStringBase
	port := varIntPort
	metricsPort := varMetricsIntPort
	if _, err := os.Stat(etcDir); os.IsNotExist(err) {
		return generateDockerfile(goFile, base, port, version, timezone, metricsPort)
	}

	_, err = findConfig(goFile, etcDir)
	if err != nil {
		return err
	}

	if err := generateDockerfile(goFile, base, port, version, timezone, metricsPort); err != nil {
		return err
	}
...
func generateDockerfile(goFile, base string, port int, version, timezone string, metricsPort int, args ...string) error {
	var projPath string
	var err error
	if len(goFile) > 0 {
