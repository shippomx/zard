...
func ZRPC(_ *cobra.Command, args []string) error {
	...
	isGrpcGateway := len(VarStringSliceGrpcGatewayOut) > 0
	if isGrpcGateway || len(VarStringSlicevalidateOut) > 0 {
		err = generator.CopyThirdPartyFromEmbed(source)
		if err != nil {
			return err
		}
	}
	...
	if isGrpcGateway {
		ctx.IsGrpcGateway = isGrpcGateway
	}
...
func wrapProtocCmd(name string, args []string) []string {
	...
	for _, validate := range VarStringSlicevalidateOut {
		ret = append(ret, "--validate_out="+validate)
	}
	for _, grpcGateway := range VarStringSliceGrpcGatewayOut {
		ret = append(ret, "--grpc-gateway_out="+grpcGateway)
	}
	return ret
}
